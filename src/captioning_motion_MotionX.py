...
def visualize_frames_BEAT_deprecated(pose_seq_data, trans, name):
    Mesh3D, Skeleton3D = True, True
    pose_seq_data = pose_seq_data.view(pose_seq_data.shape[0], -1)
    ...
    if Mesh3D:
        ...
        imgs = utils_visu.anim_from_pose_data(pose_seq_data, body_model, viewpoints=viewpoints, color="blue")
        for i, vp in enumerate(viewpoints):
            utils_visu.img2gif(imgs[i], f'out_temp/{name}/{name}_3DM_View({i}).gif')
...
def visualize_frames_MotionX(pose_seq_data, trans, name, rot_id):
    ...
    if Mesh3D:
        ...
        imgs = utils_visu.anim_from_pose_data(pose_seq_data, body_model, viewpoints=viewpoints, color="blue")
        for i, vp in enumerate(viewpoints):
            utils_visu.img2gif(imgs[i], f'out_temp/{name}/{rot_id}_3DM_View({i}).gif')
    ...
    if Skeleton3D:
        ...
        if pose_seq_data.shape[1]>66: # AMASS and BEAT
            j_seq = body_model(pose_body=pose_seq_data[:, 3:66],
                               pose_hand=pose_seq_data[:, 66:],
                               root_orient=pose_seq_data[:, :3]).Jtr
        else:
            j_seq = body_model(pose_body=pose_seq_data[:, 3:66],
                               # pose_hand=pose_seq_data[:, 66:], HumanML3D has no finger
                               root_orient=pose_seq_data[:, :3]).Jtr
        ...
        title = [""]
        # motion_batchified = torch.unsqueeze(motions, 0)
        utils_visu.draw_to_batch_Payam(motion_batchified, title, [f'out_temp/{name}/{rot_id}_3DS.gif'])


# --------- Cut video for Motion-X dataset
import cv2
...
def generate_text(file_index):
    ...
    with torch.no_grad():
        j_seq_3D_coord = body_model(pose_body=pose_seq_data[:, 3:66],
                                    pose_hand=pose_seq_data[:, 66:],
                                    root_orient=pose_seq_data[:,
                                                :3]).Jtr  # U should decide about the root orentation that I want to use
...
def generate_text_HumanML3D(motion_id, motion_path, root_euler_path, start_frame=None, end_frame=None,
                            motion_stats=False, ablations=[]):
    import argparse
    from text2pose.config import POSESCRIPT_LOCATION
    ...
    (FirstPerson_pose_seq_data, pose_seq_data,
     root_rot_vec, trans) = utils.get_pose_sequence_data_from_file_HumanML3D_T2M_GPT(motion_path,
                                                                                     root_euler_path,
                                                                                     normalizer_frame=start_frame)
    fps = 20.0
    ...
    # start_frame, end_frame = 0, len(pose_seq_data)  # 40 # len(pose_seq_data)
    ...
    root_euler_orient[:, 2] = normalize_angles(
        (root_euler_orient[:, 2] - angle_diff))  # not sure if this is correct for negatives
    ...
    # Check: _________________________________
    # trans = np.zeros_like(trans)
    # visualize_frames_HumanML3D(pose_seq_data, trans, motion_id)
    # exit()
    ...
    if args.action == "generate_captions":
        ...
        if args.add_babel_info:
            ...
            pose_babel_text = captioning_py.create_sentence_from_babel_tags(pose_babel_tags, babel_tag2txt)
            '''
            motion_babel_text, motion_babel_details = captioning_py.create_sentence_from_babel_to_hml3d(HML3D_BABEL,
                                                                                                        babel_tag2txt,
                                                                                                        motion_id,
                                                                                                        start_time,
                                                                                                        end_time,
        ...
        if Ablation_No_motionScript:
            ...
            binning_detial, motion_descriptions_non_agg, motion_description = captioning_py.main(coords,
                                                                                                 save_dir=save_dir,
                                                                                                 babel_info=pose_babel_text,
                                                                                                 simplified_captions=args.simplified_captions,
                                                                                                 apply_transrel_ripple_effect=args.apply_transrel_ripple_effect,
...
def generate_text_MotionX(motion_id, motion_path, video_path, start_frame=None, end_frame=None,
                            motion_stats=False, ablations=[]):
    import argparse
    from text2pose.config import POSESCRIPT_LOCATION
    ...
    # motion_path = r"S:\Payam\Gesture_Generation\PantoMatrix\scripts\EMAGE_2024\BEAT2\beat_english_v2.0.0\smplxflame_30\2_scott_0_1_1.npz"
    # # motion_path = 'C:\\Users\\payam.jomeyazdian\\Downloads\\SMPL-X(Andrew)-20240328T000100Z-001\\SMPL-X(Andrew)\\SMPLX\\BEAT_LIKE.npz'
    # motion_path = r'S:\Payam\Motion-X\data\motion\motion_generation\smplx322\idea400\motion_generation\smplx_322\idea400\Opening_the_Box_and_walking_at_the_same_time_clip1.npy'
    # video_path = r'S:\Payam\Motion-X\data\video\idea400_light\Opening_the_Box_and_walking_at_the_same_time_clip1.mp4'
    (FirstPerson_pose_seq_data, pose_seq_data, root_rot_vec, trans) = (utils.get_pose_sequence_data_from_file_MOTIONX
                                                                                    (motion_path,

                                                                  normalizer_frame=start_frame))
    # cut_video(input_path=video_path, output_path=f'out_temp/{motion_id}/video.mp4', start_time=0, end_time=274)
    ...
    # combinations = []
    # pose_seq_data, trans = pose_seq_data[:], trans[:]
    # for first_rotation in rotations:
    #     for first_degree in degrees:
    #         for first_sign in signs:
    #             if first_degree == '0' and first_sign == '-':
    #                 continue  # Skip the combination with negative sign for zero degree
    #             for second_rotation in rotations:
    #                 if second_rotation == first_rotation:
    #                     continue  # Skip rotating around the same axis twice
    #                 for second_degree in degrees:
    #                     for second_sign in signs:
    #                         if second_degree == '0' and second_sign == '-':
    #                             continue  # Skip the combination with negative sign for zero degree
    #                         if second_rotation == first_rotation:
    #                             continue  # Skip rotating around the same axis twice
    #                         for third_rotation in rotations:
    #                             if third_rotation == first_rotation or third_rotation == second_rotation:
    #                                 continue  # Skip rotating around the same axis twice
    #                             for third_degree in degrees:
    #                                 for third_sign in signs:
    #                                     if third_degree == '0' and third_sign == '-':
    #                                         continue  # Skip the combination with negative sign for zero degree
    #                                     combination = f"{first_sign}{first_degree}{first_rotation}_{second_sign}{second_degree}{second_rotation}_{third_sign}{third_degree}{third_rotation}"
    #                                     combinations.append(combination)
    # degrees = [0]
    # combinations = []
    # for x_deg in degrees:
    #     for y_deg in degrees:
    #         for z_deg in degrees:
    #             combination = f"{x_deg}x_{y_deg}y_{z_deg}z"
    #             combinations.append(combination)
    # for index_xx, xx in enumerate(combinations):
    #     three_rots = xx.split('_')
    #
    #     j_seq = copy.deepcopy(pose_seq_data)
    #     j_seq = j_seq.view(pose_seq_data.shape[0], -1, 3)
    #     for rot in three_rots:
    #         if 'x' in rot: current_rot = rotX
    #         if 'y' in rot: current_rot = rotY
    #         if 'z' in rot: current_rot = rotZ
    #         current_deg = int(rot.replace('x', '').replace('y', '').replace('z', ''))
    #         for frame_i in range(j_seq.shape[0]):
    #             j_seq[frame_i, 0, :] = transf(current_rot, current_deg, j_seq[frame_i, 0, :].unsqueeze(0)).squeeze()
    #
    #     j_seq = j_seq.view(pose_seq_data.shape[0], -1)
    #     visualize_frames_MotionX(j_seq, trans, motion_id, str(index_xx)+'_'+xx)
    ...
    #     return '', '', 0.0, 0.0
    # SEQ_LENGTH = min(int(AUGMENTATION_LENGTH * fps), len(pose_seq_data))
    SEQ_LENGTH = min(int(3 * fps), len(pose_seq_data))
    # SEQ_LENGTH = len(pose_seq_data)

    if len(pose_seq_data) < fps / 2:
        return '', '', '', 0, 0
    start_frame = random.randint(0, len(pose_seq_data) - SEQ_LENGTH)  # since both included
    end_frame = start_frame + SEQ_LENGTH
    start_frame, end_frame = int(0),  int(2*fps) # len(pose_seq_data) # int(6.5*20 )# len(pose_seq_data) X#1
    start_frame, end_frame = 0, 40 # len(pose_seq_data)  # 40 # len(pose_seq_data)
    ...
    # Todo: ----------- rotation vectors to 3D coordinates
    # pose_seq_data = pose_seq_data[:end_frame] #todo: This is necessary for the MotionX sice it's rotation_vec, not location
    pose_seq_data_shape = pose_seq_data.shape
    # infer coordinates
    # Todo: ----------- rotation vectors to 3D coordinates
    with torch.no_grad():
        j_seq_3D_coord = body_model(pose_body=pose_seq_data[:, 3:66],
                                    ...
                                    root_orient=pose_seq_data[:,:3]).Jtr  # U should decide about the root orentation that I want to use
    # FirstPerson_pose_seq_data = FirstPerson_pose_seq_data.reshape(pose_seq_data.shape[0], -1, 3)
    FirstPerson_pose_seq_data = j_seq_3D_coord.reshape(pose_seq_data.shape[0], -1, 3)
    for frame_i in range(FirstPerson_pose_seq_data.shape[0]):
        #     # break
        #
        trans_oriented = torch.tensor(trans[frame_i]).unsqueeze(0).float().to(device)

        FirstPerson_pose_seq_data[frame_i] = transf(rotX, -90, FirstPerson_pose_seq_data[frame_i])
        trans_oriented = transf(rotX, -90, trans_oriented)
        # FirstPerson_pose_seq_data[frame_i] += trans_oriented
    ...
    coords = pose_seq_data
    # # coords = normalize_coords(pose_seq_data, trans)
    coords = FirstPerson_pose_seq_data[:, :22, :] # Exlude fingers for now
    ...
    for frame in range(root_euler_orient.shape[0]):
        ...
        root_euler_orient[frame, :] = torch.cat((torch.unsqueeze(rad2deg(theta_x), 0),
                                                 torch.unsqueeze(rad2deg(theta_y), 0),
                                                 torch.unsqueeze(rad2deg(theta_z), 0))).squeeze()
        # Todo: -90 degree around the X-Axis
        root_euler_orient[frame, :] = torch.cat((torch.unsqueeze(rad2deg(theta_x), 0),
                                                 torch.unsqueeze(rad2deg(theta_y), 0),
                                                 torch.unsqueeze(-rad2deg(theta_z), 0))).squeeze()
    ...
    # In order to be compatible with motionscript we add zeros for fingers
    # Later, they would be ignored in the prepare_input() function
    # Since this function is working with BEAT SMPL-X dataset we do not need it
    coords = torch.cat((coords, torch.zeros(coords.shape[0], 2 * 15, 3).to(coords.device)), dim=1)
    ...
    for j in [52]:
        ...
        plt.legend()
        # plt.show()
        plt.savefig(f'out_temp/{motion_id}/temp_plot({j}).png')
        plt.clf()
    # exit()
    # Calculate the differences between consecutive elements
    if False:
    ...
    # # visualize_frames_MotionX(pose_seq_data, trans, motion_id)
    visualize_frames_MotionX(j_seq, trans, motion_id, motion_id)
    cut_video(input_path=video_path, output_path=f'out_temp/{motion_id}/video.mp4', start_time=start_time, end_time=end_time)
...
motionx_path_root = r'S:\Payam\Motion-X\data'
motionx_path_video = r'video\idea400_light'
motionx_path_smplx = r'motion\motion_generation\smplx322\idea400\motion_generation\smplx_322\idea400'
motionx_path_semantical = r'text\semantic_label\idea400\idea400'

motionx_ids =   [os.path.basename(file)[:-4] for file in glob.glob(os.path.join(motionx_path_root, motionx_path_video, '*.mp4'))]


for sample_index in tqdm.tqdm(range(500, 600)): # len(motionx_ids))):
    id = motionx_ids[sample_index]
    # id = motionx_ids[5001]
    print(id)
    # id = 'Simultaneously_walking_and_Dance_clip2'
    # id = 'Simultaneously_walking_and_Bundi_clip1'
    counter += 1
    # id = '000406'
    ...
    motion_path = os.path.join(motionx_path_root, motionx_path_smplx, f'{id}.npy')
    # euler_oath = None #f'{humanml3d_path}\\new_joint_vecs\\{id}.npy'
    org_caption = open(os.path.join(motionx_path_root,motionx_path_semantical, f'{id}.txt'), 'r', encoding='utf-8').read()
    vide_path = os.path.join(motionx_path_root, motionx_path_video, f'{id}.mp4')
    ...
    if True:
        ...
        for caption_id in range(1): #(AUGMENTATION_PORTION):

            # (binning_detial, generated_motion_description_non_agg, generated_motion_description,
            #  start_time, end_time) = generate_text_HumanML3D(id, motion_path, root_euler_path,
            #                                                  motion_stats=motion_stats, ablations=[])

            (binning_detial, generated_motion_description_non_agg, generated_motion_description,
             start_time, end_time) = generate_text_MotionX(id, motion_path, vide_path,
                                                             motion_stats=motion_stats, ablations=[])
            ...
            current_sanity_text = (
                        f'Start: {start_time:.1f}s ---- End: {end_time:.1f}s' + '\n' * 3 + "Binning details:\n" +
