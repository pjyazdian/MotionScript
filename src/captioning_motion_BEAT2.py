...
def visualize_frames_BEAT(pose_seq_data, trans, name):
    Mesh3D, Skeleton3D = True, True
    pose_seq_data =pose_seq_data.view(pose_seq_data.shape[0], -1)
    # 3D Mesh Visualization Animation doesn't work for HumanML3D for now
    if Mesh3D:
...
def generate_text_BEAT2(motion_id, motion_path, root_euler_path, start_frame=None, end_frame=None,
                            motion_stats=False, ablations=[]):
    import argparse
    from text2pose.config import POSESCRIPT_LOCATION
    ...
    # motion_path = r'S:\Payam\Motion-X\data\motion\motion_generation\smplx322\idea400\motion_generation\smplx_322\idea400\Act_cute_and_sitting_at_the_same_time_clip1.npy'
    (FirstPerson_pose_seq_data, pose_seq_data, root_rot_vec, trans) = (utils.get_pose_sequence_data_from_file_BEAT2T
                                                                                    (motion_path,
                                                                                     root_euler_path,
                                                                                     normalizer_frame=start_frame))
    ...
    #     return '', '', 0.0, 0.0
    # SEQ_LENGTH = min(int(AUGMENTATION_LENGTH * fps), len(pose_seq_data))
    SEQ_LENGTH = min(int(5 * fps), len(pose_seq_data))
    ...
    start_frame = random.randint(0, len(pose_seq_data) - SEQ_LENGTH)  # since both included
    end_frame = start_frame + SEQ_LENGTH
    start_frame, end_frame = int(0),  int(10*fps) # len(pose_seq_data) # int(6.5*20 )# len(pose_seq_data) X#1
    # start_frame, end_frame = 0, len(pose_seq_data)  # 40 # len(pose_seq_data)
    ...
    #     plt.legend()
    #     # plt.show()
    #     plt.savefig(f'out_temp/temp_plot({j}).png')
    #     plt.clf()
    # exit()
    # Calculate the differences between consecutive elements
    if False:
    ...
    visualize_frames_BEAT(pose_seq_data, trans, motion_id)
...
for sample_index in tqdm.tqdm(range(0 * len(lines) // 5, 1 * len(lines) // 5)):
    ...
    # id = '000028'
    # todo: to fix
    # id = '014112'  # need redifine finner z rotation
    # id = '013260'
    # id = '013022'
    # id = '001426'
    # id = '014290'
    ...
    # id = '000264'
    # id = '003470'
    # id = '004972'   # Z rotation APPROVED
    # id = '005876' # step to the left
    # id = '010162'
    # id = '013015' # z= -90 facing forward, +90 facing backward 0 facing to left
    # id = '012696'
    # id = '012136'
    # id = '001426'
    ...
    # id = '000204'
    # id = '001303'
    # id = '002604' # right-to-left sanity check
    # id = '003610' 35 frames
    # id = '004819'
    # id = '004678'   # Angelica
    # id = '001282' # Dance
    ...
    if True:
        ...
        for caption_id in range(1): #(AUGMENTATION_PORTION):
            ...
            (binning_detial, generated_motion_description_non_agg, generated_motion_description,
             start_time, end_time) = generate_text_BEAT2(id, motion_path, root_euler_path,
                                                             motion_stats=motion_stats, ablations=[])
