...
from MS_Algorithms.py import create_gif_with_blinking, merge_gifs_side_by_side
...
def visualize_frames_HumanML3D(pose_seq_data, trans, Motioncodes4vis, name, org_path):
    Mesh3D, Skeleton3D, Motioncodes, Merge = False, True, True, True
    pose_seq_data =pose_seq_data.view(pose_seq_data.shape[0], -1)
    # 3D Mesh Visualization Animation doesn't work for HumanML3D for now
    if Mesh3D:
        print("Plotting 3D Mesh...")
        # This is a trick borrowed from T2M-GPT render_final.py to visualize using joint positions
        motions_HML3D_Raw = np.load(org_path.replace("new_joint_vecs", "new_joints"))
        utils_visu.renderT2M_GPT(motions_HML3D_Raw, outdir=f'out_temp/{name}/', device_id=0, name=f'{name}_3DMX', pred=False)
        # viewpoints = [(90, (1, 0, 0))]
        # imgs = utils_visu.anim_from_pose_data(pose_seq_data, body_model, viewpoints=viewpoints, color="blue")
        # for i, vp in enumerate(viewpoints):
...
def generate_text_HumanML3D(motion_id, motion_path, root_euler_path, start_frame=None, end_frame=None, motion_stats=False, ablations=[]):
    ...
    if args.action == "generate_captions":
        ...
        if Ablation_No_motionScript:
            ...
            # visualize_frames_HumanML3D(pose_seq_data, trans, motioncodes4vis, motion_id, motion_path)
